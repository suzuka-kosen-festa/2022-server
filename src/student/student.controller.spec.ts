...
describe('StudentController', () => {
   ...
   beforeEach(async () => {
      const module: TestingModule = await Test.createTestingModule({
         ...
         providers: [{
            ...
            useValue:{
               createStudent: jest.fn().mockImplementation((data: Prisma.StudentCreateInput) =>
               Promise.resolve(data)
               ),
               checkStudentExist: jest.fn().mockImplementation((uuid : string) => Promise.resolve(singleRecord)),
               updateStudent: jest.fn().mockImplementation((data: Prisma.StudentUpdateInput) => Promise.resolve(singleRecordwithGuest)),
   ...
   test('update Student',async () => {
      const data = await controller.updateStudent({email:'example2.com', sex:'男', jobs:'祖父',RealName:'テストゲスト'})
